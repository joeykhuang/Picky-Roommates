...
def get_ef_df(m, partition, prefs):
    ...
    for i in range(1, m + 1):
        ...
        for j in range(1, m + 1):
            # envy df might be wrong; TODO: fix
...
def find_ef_price(m, partition, person_rm_table, envy_df, inv_epsilon):
    prices = range(m)
    model = gp.Model()
    ...
    for i in range(m):
        for j in range(i + 1, m):
            ...
            if j != i_rm and i != j_rm:
                # PEF
                model.addConstr(envy_df.iloc[i, i_rm] - p[i] >= inv_epsilon * (envy_df.iloc[i, j_rm] - p[j]))
                # REF
...
def simulate_ef_prices(m, n, num_simulations, inv_epsilon):
    ...
    for _ in range(num_simulations):
        ...
        greedy_person_rm_table, _, greedy_envy_df = get_ef_df(m, greedy_match_partition, prefs)
        greedy_prices = find_ef_price(m, greedy_match_partition, greedy_person_rm_table, greedy_envy_df, inv_epsilon)

        if len(greedy_prices) > 0:
            greedy_match_found_sol += 1
            greedy_match_results.append(greedy_prices)

        mwis_person_rm_table, _, mwis_envy_df = get_ef_df(m, mwis_partition, prefs)
        mwis_prices = find_ef_price(m, mwis_partition, mwis_person_rm_table, mwis_envy_df, inv_epsilon)
